...
df = df[["year","date","radius","dif_lat","dif_lon","trigger","countrycode"]]
...
for index, row in df.iterrows():
    if lastTrigger == row["trigger"] and (row["date"] - lastDate).days < 3:
        distance = np.sqrt(row["dif_lat"]**2  +  row["dif_lon"]**2)
        if lastRadius != "nan" or row["radius"] != "nan" and (distance < row["radius"] or distance < lastRadius):
...
plt.xlabel("Jahr")
plt.ylabel("Anzahl")
plt.title("Registrierte Erdrutsche (gefiltert, Radius <= "+str(a)+" km)")
plt.show()